...
namespace WC
{
	public class CharacterHorizontalMovement : CharacterPhysicsModule
	{
		...
		public override void Affect(IPhysics physics)
		{
			...
			// Defying new velocity as old velocity + acceleration towards desired velocity
			...
			// Defying resistance to movement towards slope
			...
			if (Vector3.Angle(_characterGravity.WorldUp, groundNormal) > 0.001f)
			{
				...
				// Velocity, that fully negotiate movement towards slope
				Vector3 fullSlopeResistance = Vector3.Project(velocity, slope);

				// Defying resistance factor from slope angle
				float resistanceToWorldUpAngle = fullSlopeResistance.sqrMagnitude > 0.001f
					...
					: 90f;
				float resistanceFactor = Mathf.Clamp01(Mathf.InverseLerp(_tolerantSlope, _maxSlope, 90f - resistanceToWorldUpAngle));

				slopeResistance = fullSlopeResistance * resistanceFactor;
			}
